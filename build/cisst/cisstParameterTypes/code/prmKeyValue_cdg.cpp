...
// cisst version: 1.4.0
// source file: /home/utc_sim/ros2_dvrk/src/cisst-saw/cisst/cisstParameterTypes/code/../prmKeyValue.cdg
...
void prmKeyValue::DeSerializeTextJSON(const Json::Value & jsonValue) CISST_THROW(std::runtime_error) {
    ...
    if (!field__cdg.empty()) {
        try {
            ...
            cmnThrow(std::string(e.what()) + " < Key");
        }
    } else {
    ...
    if (!field__cdg.empty()) {
        try {
            ...
            cmnThrow(std::string(e.what()) + " < Value");
        }
    } else {
