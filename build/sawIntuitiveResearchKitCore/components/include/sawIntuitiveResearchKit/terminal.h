...
#pragma once
#ifndef _sawIntuitiveResearchKit_terminal_h
#define _sawIntuitiveResearchKit_terminal_h
...
/* source line: 25 */
 ...
 public:
    void Copy(const terminal & source);
...
typedef mtsGenericObjectProxy<dvrk::terminal> dvrk_terminalProxy;
CMN_DECLARE_SERVICES_INSTANTIATION(dvrk_terminalProxy);

/* default functions */
void CISST_EXPORT cmnSerializeRaw(std::ostream & outputStream, const dvrk::terminal & object);
void CISST_EXPORT cmnDeSerializeRaw(std::istream & inputStream, dvrk::terminal & placeHolder);
/* data functions */
template <> class cmnData<dvrk::terminal > {
public: 
    enum {IS_SPECIALIZED = 1};
    typedef dvrk::terminal DataType;
...
inline std::ostream & operator << (std::ostream & outputStream, const dvrk::terminal & data) {
    outputStream << cmnData<dvrk::terminal >::HumanReadable(data);
    return outputStream;
}
#if CISST_HAS_JSON
template <> void CISST_EXPORT cmnDataJSON<dvrk::terminal >::SerializeText(const dvrk::terminal & data, Json::Value & jsonValue);
template <> void CISST_EXPORT cmnDataJSON<dvrk::terminal >::DeSerializeText(dvrk::terminal & data, const Json::Value & jsonValue) CISST_THROW(std::runtime_error);
#endif // CISST_HAS_JSON


#endif // _sawIntuitiveResearchKit_terminal_h
